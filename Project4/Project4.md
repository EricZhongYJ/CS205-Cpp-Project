...
可以看出，方法`matmul_improved`比方法`matmul_improved_sa`快，且均比方法`matmul_plain`快，它们的结果差的平均值为0，可以说明计算的准确性，耗时分析参考Part 4问题.
...
由图可知，拟合曲线与原数据几乎完全重合，拟合效果较好。其中最快的方法(14.817s)均慢于下面`-O3`优化后的最慢的运行结果(12.742s)。
