...
#else // Windows只支持cd
#define __CLEAR "cls"
#define __run "%c: & cd %s & %s", curStr[0]
#define __mkdir "%c: & cd %s & mkdir %s", curStr[0]
#define __touch "%c: & cd %s & cd .>%s", curStr[0]
#define __cat "%c: & cd %s & type %s", curStr[0]
#define __move "%c: & cd %s & move %s %s", curStr[0]
#define __copy "%c: & cd %s & copy %s %s", curStr[0]
#define __vim "%c: & cd %s & notepad %s", curStr[0]
#define __deldir "%c: & cd %s & rmdir /s /q %s", curStr[0]
#define __del "%c: & cd %s & del %s", curStr[0]
#endif
...
int main(int argc, char const *argv[])
{
    ...
    START: // 清屏并输出提示
        ...
        cout << "当前文件夹为:\n"
             << curStr << endl;
        if (showFile)
            cout << "当前文件夹下文件/文件夹有" << fileN << "个:\n  文件(夹)名\t\t\t\t文件类型\t 文件大小(字节)" << endl;
        for (int i = 0; i < fileN && showFile; ++i)
        {
            if (names[i].size() < 35)
            {
                strcpy(oup, "                                   %s%18d\n\0");
        ...
        cout << "请输入指令(按错指令输入'b'返回, 对操作e:写入文件内容输入第二行后无效):" << endl
             ...
             << "\t5:新建文件夹\t\t6:新建文件" << endl
             << "\t7:重命名文件(夹)\t8:(不)显示具体文件" << endl
             << "\t9:编译运行c++文件并删除可执行文件(多个文件从第二个起请输入绝对路径)" << endl
             << "\tq:退出\t\t\tw:查看文件" << endl
             << "\te:写入文件\t\tr:编辑文件" << endl
        ...
        if (inp[1] == '\0')
            ...
            case '8': // (不)显示具体文件
                showFile = !showFile;
                cout << (showFile ? "显示具体文件！" : "不显示具体文件！") << endl;
                __Back;
            case '9': // 编译运行c++文件并删除可执行文件
